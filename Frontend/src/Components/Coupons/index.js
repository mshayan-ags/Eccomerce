...
const DiscountCoupon = ({ data, coupon_sale, setCoupon, noButton, getTotal, Coupon, GetAllCouponsUser }) => {
  ...
  // In display-only mode (an already-placed order's coupon summary) the
  // coupon is a historical fact, not something still being offered - skip
  // the "can this still be applied" eligibility checks, which would
  // otherwise make the coupon disappear once it expires.
  ...
  if (isEligibleToApply) return (
      <div data-aos="fade-left" data-aos-duration="3000" data-aos-delay="800" className="w-80 rounded-lg p-2 m-6 text-center bg-[#FFFFFF] shadow-lg flex flex-col justify-center align-center items-center border-[2px] border-dashed border-[#595959]">
        ...
        <p className="text-[#DC143C] mt-1 text-xs font-bold">Valid Until: <span className='text-[#595959] font-bold text-[14px]'>{moment(data?.expirationDate)?.format("DD MMMM YYYY, hh:mm A")}</span></p>
