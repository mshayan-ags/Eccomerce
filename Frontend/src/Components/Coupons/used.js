...
const UsedDiscountCoupon = ({ Coupon, setCoupon, GetUsedCouponsUser, UsedCoupon }) => {
  ...
  useEffect(() => {
    if (Coupon) GetUsedCouponsUser()
  }, [Coupon]);

  if (!UsedCoupon?._id) {
    // Still loading, or nothing applied yet - not necessarily an error.
    return null;
  }
  ...
  return (
    <div data-aos="fade-left" data-aos-duration="3000" data-aos-delay="1000" className="w-80 border-2 border-dashed border-black rounded-lg p-2 my-1 text-center bg-white shadow-lg">
      ...
      <p className="text-gray-600 text-xs">Valid Until {moment(UsedCoupon?.expirationDate)?.format("DD MMMM YYYY, hh:mm A")}</p>
      <div className="mt-[10px] mb-[5px] bg-[#002A48] h-[35px] w-[100%] rounded-[25px] box-border flex flex-row items-center justify-center pt-3.5 pb-2.5 pr-6 pl-5 gap-[0px_10px] min-w-[114px] border-[2px] border-solid border-[#1e8a30ff]">
